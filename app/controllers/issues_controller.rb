...
class IssuesController < ApplicationController
  ...
  def retrieve_previous_and_next_issue_ids
    if params[:prev_issue_id].present? || params[:next_issue_id].present?
      ...
      @prev_issue_id = @issues.where('issues.id > ?', @issue.id).order('id asc').limit(1).pluck(:id).first
      @next_issue_id = @issues.where('issues.id < ?', @issue.id).order('id desc').limit(1).pluck(:id).last
    end
  end
  ...
  def redirect_after_create
    if params[:continue]
      ...
      attrs.merge!(Setting.default_copy_custom_fields.reduce({custom_field_values: {}}) { |acc, cf| acc[:custom_field_values][cf] = @issue.custom_field_value(cf); acc  }) if Setting.default_copy_custom_fields

...
class User < Principal
  ...
  def projects_by_role
    ...
    members_userid = Member.joins(:project, :principal).
      where("#{Project.table_name}.status <> 9").
      where("#{Member.table_name}.user_id = ?", self.id).
      preload(:project, :roles).
      to_a

    members_public =  Member.joins(:project, :principal).
      where("#{Project.table_name}.status <> 9").
      where("(#{Project.table_name}.is_public = ? AND #{Principal.table_name}.type = ?)", true, group_class.name).
      preload(:project, :roles).
      to_a

    members = members_userid + members_public
  ...
  safe_attributes 'status',
    ...
    :if => lambda {|user, current_user| current_user.admin? && user != current_user}
  ...
  def admin_can?(permission)
    !admin_restriction || !admin_restriction.restrictions.include?(permission.to_s)
  end

  def admin_can_self_manage_membership?(user)
    if !self.admin? || self.admin_can?(:self_manage_membership)
      true
    elsif user.is_a?(User) && user != self
      true
    elsif user.is_a?(Group) && !user.users.include?(User.current)
      true
    else
      false
    end

...
class IssueQuery < Query
  ...
  def initialize_available_filters
    ...
    if project
      principals += Principal.assignable_member_of(project.self_and_descendants).visible
      principals_creators += Principal.member_of(project.self_and_descendants).visible
      versions = project.shared_versions.to_a
      categories = project.issue_categories.to_a
      issue_custom_fields = project.all_issue_custom_fields
    else
      if all_projects.any?
        principals += Principal.assignable_member_of(all_projects).visible
        principals_creators += Principal.member_of(all_projects).visible
      end
    ...
    principals_creators.uniq!
    principals_creators.sort!
    principals_creators.reject! {|p| p.is_a?(GroupBuiltin)}
    users = principals.select {|p| p.is_a?(User)}
    users_creators = principals_creators.select {|p| p.is_a?(User)}
    ...
    author_values += users_creators.collect{|s| [s.name, s.id.to_s] }

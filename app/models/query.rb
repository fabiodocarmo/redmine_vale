  ...
  def sql_for_field(field, operator, value, db_table, db_field, is_custom_filter=false)
    ...
    when "="
      if value.any?
        ...
        if is_custom_filter
          value_hashed = Array.new
          if value.present?
            value.each do |v|
              value_hashed << Digest::SHA256.hexdigest(v)
            end
          end
          if value_hashed.size > 0
            sql = sql +' AND '+ queried_class.send(:sanitize_sql_for_conditions, ["#{db_table}.#{db_field}_hashed IN (?)", value_hashed])
          end
        end
    ...
    when "!"
      if value.any?
        ...
        if is_custom_filter
          ...
          if value_hashed.size > 0
            sql = sql +' AND '+ queried_class.send(:sanitize_sql_for_conditions, ["#{db_table}.#{db_field}_hashed NOT IN (?)", value_hashed])
          end
        end
      else
        # NOT IN an empty set

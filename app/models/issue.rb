  ...
  def self.visible_condition(user, options={})
    Project.allowed_to_condition(user, :view_issues, options) do |role, user|
      sql = if user.id && user.logged?
        ...
        when 'own_watched'
          user_ids = [user.id] + user.groups.map(&:id).compact
          "(#{table_name}.author_id = #{user.id} OR #{table_name}.assigned_to_id IN (#{user_ids.join(',')}) OR #{table_name}.id IN (SELECT #{Watcher.table_name}.watchable_id FROM #{Watcher.table_name} WHERE #{Watcher.table_name}.watchable_type = '#{self.name}' AND #{Watcher.table_name}.user_id IN (#{user_ids.join(',')})))"  #{Watcher.table_name}.user_id = ?
        else
          '1=0'
        end
      else
  ...
  def workflow_rule_by_attribute(user=nil)
    ...
    if workflow_permissions.any?
      ...
      roles.each do |role|
        fields_with_roles.each do |field_id, role_ids|
          unless role_ids.include?(role.id)
            ...
            workflow_rules[field_name][role.id] = 'hidden'
          end
        end
      end
      workflow_rules.each do |attr, rules|
        ...
        if 'required'.in? uniq_rules
          result[attr] = 'required'
        elsif 'readonly'.in? uniq_rules

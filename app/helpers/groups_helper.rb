...
module GroupsHelper
  def group_settings_tabs(group)
    ...
    if User.current.admin_can_self_manage_membership?(group)
  ...
  def render_principals_for_new_group_users(group, limit=100)
    ...
    unless User.current.admin_can_self_manage_membership?(User.current)
      scope = scope.where('users.id != ?', User.current.id)
    end

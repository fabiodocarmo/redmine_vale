...
class IssuesControllerTest < ActionController::TestCase
  ...
  def test_index_with_relations_column
    ...
    assert_select "tr#issue-1 td.relations" do
      ...
      assert_select "span", :text => "Related to #{Issue.find(7).project} #7"
      assert_select "span", :text => "Related to #{Issue.find(8).project} #8"
      assert_select "span", :text => "Blocks #{Issue.find(11).project} #11"
    end
    assert_select "tr#issue-2 td.relations" do
      assert_select "span", 1
      assert_select "span", :text => "Blocked by #{Issue.find(12).project} #12"
    end
  ...
  def test_get_new_with_no_tracker_should_display_an_error
    ...
    get :new, :project_id => 1
    assert_response 500
    assert_select_error /No issue type/
  end
  ...
  def test_put_update_with_project_change
    ...
    assert_mail_body_match "Area changed from eCookbook to OnlineStore", mail
  end
  ...
  def test_put_update_with_tracker_change
    ...
    assert_not_nil mail
    assert mail.subject.starts_with?("[#{issue.project.name} - #{issue.tracker.name} ##{issue.id}]")
    assert_mail_body_match "Issue type changed from Bug to Feature request", mail
  end

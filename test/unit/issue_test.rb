...
#                          :relation_type => IssueRelation::TYPE_DUPLICATES)
  ...
  def test_overdue
    assert Issue.new(:due_date => 2.day.ago.to_date).overdue?
    assert !Issue.new(:due_date => Date.today).overdue?
    assert !Issue.new(:due_date => 1.day.from_now.to_date).overdue?
    assert !Issue.new(:due_date => nil).overdue?
    assert !Issue.new(:due_date => 2.day.ago.to_date,
  ...
  test "#assignable_users should not include the current assignee" do
    user = User.generate!

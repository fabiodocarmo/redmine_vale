...
class QueriesHelperTest < ActionView::TestCase
  ...
  def test_filters_options_for_select_should_group_associations_filters
    ...
    with_locale 'en' do
      options = filters_options_for_select(IssueQuery.new)
      assert_select_in options, 'optgroup[label=?]', 'Responsible Area', 1
      assert_select_in options, 'optgroup[label=?] > option', 'Responsible Area', 2
      assert_select_in options, 'optgroup > option[value=?]', "project.cf_#{cf1.id}", :text => "Area's Foo"
    end
  end

class ExportToValeCteOsRobotController < ApplicationController
  unloadable
  ...
  def create
    issues = ValeExportReplicaIssue.where(project_id: @project.id)
                  .where(status_id: Setting.plugin_export_to_vale_sap_robot['cte_os_wating_status_id'])
                  .where(tracker_id: Setting.plugin_export_to_vale_sap_robot['cte_os_tracker_id'])
    ...
    filename = ExportToValeSapRobot::Services::ImportNfs.export_cte_os_template(issues, ['ID',
                                                                                         'OBSERVACOES',
                                                                                         'CNPJ_VALE',
                                                                                         'OPERACAO',
                                                                                         'CNPJ_FORNECEDOR',
                                                                                        ...
                                                                                        ['ID_LINHA',
                                                                                         ...
                                                                                         'TAXA_IMPOSTO',
                                                                                         'FLAG_MONTANTE_TAXA'])
    send_file(filename, filename: 'template_robo_cte_os.xlsx', type: "application/vnd.ms-excel")

    Issue.where(id: issues.map(&:id)).each do |i|
      i.init_journal(User.current)
      i.update_attribute(:status_id, Setting.plugin_export_to_vale_sap_robot['cte_os_robot_status_id'])
    end
  end
  ...
  def update_ok
    ...
    journal_msg_col = nil
    custom_field_cols = []
    custom_field_cols << {id: Setting.plugin_export_to_vale_sap_robot['cte_os_miro'], col: -2}
    current_status_id = Setting.plugin_export_to_vale_sap_robot['cte_os_robot_status_id']
    new_status_id = Setting.plugin_export_to_vale_sap_robot['cte_os_success_robot_status_id']
  ...
  def update_error
    ...
    custom_field_cols = []
    current_status_id = Setting.plugin_export_to_vale_sap_robot['cte_os_robot_status_id']
    new_status_id = Setting.plugin_export_to_vale_sap_robot['cte_os_error_robot_status_id']
    flash_hash = ExportToValeSapRobot::Services::ImportNfs.
      update_issues_by_log(params, log_type, issue_id_col, current_status_id, new_status_id, journal_msg_col, custom_field_cols)
  ...
  def custom_values_joins_query(table_alias, setting_id)
    custom_fields_ids = Array.wrap(Setting.plugin_export_to_vale_sap_robot["cte_os_#{setting_id}"]).join(', ')

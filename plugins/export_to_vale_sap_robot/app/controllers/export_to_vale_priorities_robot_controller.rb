class ExportToValePrioritiesRobotController < ApplicationController
  unloadable
  ...
  def create
    issues = ValeExportReplicaIssue
                  .where(status_id: Setting.plugin_export_to_vale_sap_robot['priorities_wating_status_id'])
                  .where(tracker_id: Setting.plugin_export_to_vale_sap_robot['priorities_tracker_id'])
                  ...
                  .joins(custom_values_joins_query('id_debito', 'id_debito_id'))
                  ...
                  .select('\'\' || id_debito.value as "ID Débito Automático"')
                  .select('tipo_documento.value as "Tipo de Documento"')
                  .select('issues.tracker_id as "Tracker"')
                  .select('null as "Moeda"')
                  .order(:id)

    filename = ExportToValeSapRobot::Services::ImportNfs.export_utilities_priorities_template(issues, [
                                                                                            'Empresa'             ,
                                                                                            'CNPJ Fornecedor'     ,
                                                                                            'Emissão'             ,
                                                                                            'Vencimento'          ,
                                                                                            ...
                                                                                            'RF'                  ,
                                                                                            'idsv_notas'])
    send_file(filename, filename: 'template_robo_prioritarios.xlsx', type: "application/vnd.ms-excel")

    Issue.where(id: issues.map(&:idsv_notas)).each do |i|
      i.init_journal(User.current)
      i.update_attribute(:status_id, Setting.plugin_export_to_vale_sap_robot['priorities_robot_status_id'])
    end
  end
  ...
  def update
    ...
    ImportPrioritiesJob.perform_later(@attachment.diskfile, User.current.id)
  ...
  def custom_values_joins_query(table_alias, setting_id)
    custom_fields_ids = Array.wrap(Setting.plugin_export_to_vale_sap_robot["priorities_#{setting_id}"]).join(', ')

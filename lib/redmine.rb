...
Redmine::AccessControl.map do |map|
  ...
  map.project_module :issue_tracking do |map|
    ...
    # Issue categories
    map.permission :manage_categories, {:projects => :settings, :issue_categories => [:index, :show, :new, :create, :edit, :update, :destroy]}, :require => :member
    map.restriction :view_all_issues, {}
    #Admin Restrictions
    map.restriction :manage_admin_restriction, {}
    map.restriction :give_admin, {}
    map.restriction :give_admin_restriction, {}
    map.restriction :view_all_projects, {}

    map.restriction :manage_auth_source, {}
    map.restriction :create_internal_user, {}

    map.restriction :self_manage_membership, {}
  end
...
Redmine::MenuManager.map :admin_menu do |menu|
  ...
  menu.push :admin_restrictions, {:controller => 'admin_restrictions'}, :caption => :label_admin_restriction_plural, :if => Proc.new { User.current.admin_can?(:manage_admin_restriction) }
  ...
  menu.push :ldap_authentication, {:controller => 'auth_sources', :action => 'index'},
            :html => {:class => 'server_authentication'}, :if => Proc.new { User.current.admin_can?(:manage_auth_source) }

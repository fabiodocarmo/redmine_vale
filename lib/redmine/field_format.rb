...
module Redmine
  module FieldFormat
    ...
    class Numeric < Unbounded
      field_attributes :min_value, :max_value
      ...
      def validate_single_value(custom_field, value, customized=nil)
        errs = super
        custom_field_value = cast_single_value(custom_field, value)
        errs << ::I18n.t('activerecord.errors.messages.greater_than_or_equal_to', count: custom_field.min_value) unless valid_value?(custom_field, custom_field_value, :>=, :min_value)
        errs << ::I18n.t('activerecord.errors.messages.less_than_or_equal_to', count: custom_field.max_value) unless valid_value?(custom_field, custom_field_value, :<=, :max_value)
        errs
      end

      def valid_value?(custom_field, value, operator, value_limit_attr)
        value_limit = custom_field.send(value_limit_attr)
        value_limit.blank? || (value.send(operator, cast_single_value(custom_field, value_limit)))
      end
